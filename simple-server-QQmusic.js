      ...
      if (isMusicRequest) {
        ...
        if (songName) {
          ...
          if (songs && songs.length > 0) {
            ...
            songs.forEach((song, index) => {
              replyMsg += `${index + 1}. ${song.title} - ${song.singer}`;
              if (song.url) {
                replyMsg += `\n   ${song.url}`;
              }
            ...
            replyMsg = `抱歉，没有找到「${songName}」相关的歌曲，请试试其他歌名！`;
          }
        } else {
          replyMsg = '想点什么歌？请发送：点歌 歌曲名\n\n例如：点歌 小幸运';
        }
      } else {
        ...
        if (isIdentityQuestion) {
          ...
          try {
            ...
            replyMsg = aiData.choices?.[0]?.message?.content || '收到消息！';
            replyMsg = replyMsg.replace(/\*\*/g, '').replace(/\*/g, '');
